<x-admin-layout>
    <x-slot name="header">
        Invitations {{ current_user()->username }}
    </x-slot>
    ...
    <x-card.default>
        <form action="{{ route('admin.invitations.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                <input id="invited_by" class="block mt-1 w-full" type="hidden" name="invited_by" :value="current_user()->username" readonly required />
            </div>
