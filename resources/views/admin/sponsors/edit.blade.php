<x-admin-layout>
    ...
    <x-card.default>
        <form action="{{ route('admin.sponsors.update', $sponsor->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @method('PATCH')
            @csrf
            <div class="flex justify-between gap-6">
                ...
                <div class="w-1/2">
                    ...
                    <x-form.input type="url" name="link" id="link" :value="old('name', $sponsor->link)" required  />
            ...
            <div>
                <label class="relative inline-flex items-center mr-5 cursor-pointer">
                    <input name="active"
                           id="active"
                           value="1"{{ ($sponsor->active == 1 ? ' checked' : '') }}
