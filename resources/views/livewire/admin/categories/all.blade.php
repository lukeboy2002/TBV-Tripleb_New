    ...
    @if (!$categories->isEmpty())
        <div class="overflow-x-auto relative shadow-md">
            <div class="flex justify-between items-center space-x-4 pb-4 pt-2 px-2">
                ...
                <div class="pr-2">
                    <x-link.btn-primary href="{{ route('admin.categories.create') }}" class="px-5 py-2.5 text-sm font-medium">New category</x-link.btn-primary>
                </div>
            </div>
                    ...
                    @foreach($categories as $category )
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <a href="{{ route('admin.categories.edit' , $category) }}">{{ $category->title }}</a>
                            </th>
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            ...
                            <td class="px-6 py-4 flex justify-end space-x-4">
                                <x-link.btn-primary href="{{ route('admin.categories.edit' , $category) }}" class="px-2.5 py-2.5 text-xs font-medium"><i class="fa-solid fa-pen-to-square"></i></x-link.btn-primary>
                                <x-button.danger type="button" wire:click="deleteId({{ $category->id }})" class="px-2.5 py-2.5 text-xs font-medium"><i class="fa-solid fa-trash-can"></i></x-button.danger>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
